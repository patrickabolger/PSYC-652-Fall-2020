...
#if(!require(pacman)){install.packages("pacman");require(pacman)} # un-comment and run this if the 'pacman' package is not installed. 
# It's required for the function below.
pacman::p_load(dplyr, knitr, magrittr, kableExtra, readr, tibble, tidyr)
...
|:-----------------:|:---------------------:|:----------------:|:------------------:|:-----------------:|:-------------:|
|            **Role**            |         **Name**        |     **Office**     |      **E-Mail**      |    **Zoom Hours**   |  **Zoom Link**$\dagger$ |
| Instructor                 | Patrick Bolger, PhD | Psychology 225 | pbolger@tamu.edu | Fridays, 1-3 pm | [click here](https://tamu.zoom.us/j/811222380) |


$\dagger$ See extended Zoom information under the sub-tab ***Zoom Info***
...
### Course {.tabset .tabset-fade .tabset-pills}
...
#### Software tools for data wrangling 
...
Since statistics is the math of uncertainty, there is a great deal of critical thinking involved. The classic illustration of this is [Simpson's Paradox](https://en.wikipedia.org/wiki/Simpson%27s_paradox). Statistics is a tool for scientific reasoning, but it is incomplete without critical thinking. We will encounter this often in this course.

<br/>

#### Communication  

You will learn how to communicate statistical results to other researchers in Psychology, as well as colleagues in industry. These can be **very** different audiences. 
...
Much of this class will involve (or at least permit) collaboration. We will spend some time on how this is done with modern tools In fact, this is a really important issue that often gets overlooked in the world of data science and statistics, and even more commonly in academia. In my opinion, doing things the "old-fashioned way" is a **major** contributor to workflow bottlenecks and errors.
...
hwVal <- 20 # value
hwQnt <-10 # quantity
hwTot <- hwVal*hwQnt # total (repeats below)
...
fnlVal <- 200
fnlQnt <- 1
fnlTot <- fnlVal*fnlQnt

# Attendance
atndVal <- 4
atndQnt <- 28
atndTot <- atndVal*atndQnt

gradeTot <- 1000
pntsPoss <- sum(hwTot,qzTot,fnlTot,atndTot)
```

## Tasks {.tabset .tabset-fade .tabset-pills}

There are `r gradeTot` points that count towards the final grade in this class (out of `r pntsPoss` possible). The breakdown is below:  

<br/>

### Homework

There will be `r hwQnt` homework assignments based on tutorials in either *Kanopy* or *LinkedIn Learning*. Each is worth `r hwVal` points, for a total of `r hwTot`.

<br/>

### Take-home Quizzes

There will be `r qzQnt` take-home quizzes based on the Chapters in the Flora textbook. Each is worth `r qzVal` points, for a total of `r qzTot`.  

<br/>

### Take-home Final Exam

There will be one take-home final exam, which is worth `r fnlTot` points. You will all replicate a study of your collective choosing from I/O Psychology. Each of you will end up with different data with different results. However, conceptually you can talk to each other about your findings and what they mean.

<br/>

### Attendance

This is unusual for a graduate class, but in semesters past, some students in this course decided to stop coming. You can't afford that kind of mistake. So I'm making attendance part of the grade. Each attendance is worth `r atndVal` points. There are `r atndQnt` classes, meaning `r atndTot` points. This puts us `r pntsPoss-gradeTot` points past the maximum number of points in the class, but that's okay. Do not miss these easy points. Since there are 3 "extra" classes here, I will be very strict about attendance, unless you are in dire straits.
...
|:--------------------:|:--------------------:|:--------------------:|:------------:|:---:|
| Homework             |          `r hwVal`          |          `r hwQnt`          |      `r hwTot`     |  `r (hwTot/gradeTot)*100` |
| Take-home Quizzes    |          `r qzVal`         |           `r qzQnt`          |      `r qzTot`     |  `r (qzTot/gradeTot)*100` |
| Take-home Final Exam |          `r fnlVal`         |           `r fnlQnt`          |      `r fnlTot`     |  `r (fnlTot/gradeTot)*100` |
| Attendance           |          `r atndVal`           |          `r atndQnt`          |      `r atndTot`     |  `r (atndTot/gradeTot)*100` |
| **Totals**           |                      |                      |  **1,000**$^{\dagger\dagger}$   |**100**$^{\dagger\dagger}$|

$\dagger\dagger$ Obviously, there are `r pntsPoss-gradeTot` extra points possible beyond the `r gradeTot` point limit, and these extra points come through attendance. Your point total will still be calculated out of `r gradeTot` points, though you could go over, theoretically.  
...
| Grade 	| A 	| B 	| C 	| D 	| F 	|
|:--------:	|:------------:	|:---------------:	|:---------------:	|:---------------:	|:---------------:	|
| Percents 	| 89.5 to 100% 	| 79.5 to 89.499% 	| 69.5 to 79.499% 	| 59.5 to 69.499% 	| Less than 59.5% 	|
| Points 	| `r .895*gradeTot` to  `r gradeTot`	| `r .795*gradeTot` to `r round(.89499*gradeTot,3)` 	| `r .695*gradeTot` to `r round(.79499*gradeTot,3)` 	| `r .595*gradeTot` to `r round(.69499*gradeTot,3)` 	| Less than `r .595*gradeTot` 	|
...
kable_styling(bootstrap_options = "striped", font_size=14, full_width = F, position = "left")
```
